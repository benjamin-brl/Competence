<script lang="ts">
  ...
  async function tableauLo () {
    try {
      ...
      if (errorEvaluation) {
        console.log('Problème avec la récupération des données de la table évaluation : ', errorEvaluation);
      }

      if (errorComposer) {
        console.log('Problème avec la récupération des données de la table composer : ', errorComposer);
      }
...
//* ------------------------------------------- SUPPRESSION ------------------------------------------- *//
  ...
  async function suppressionPasser(idevaluation: string) {
    try {
      ...
      const { data, error } = await supabase.from('passer').delete().eq('idevaluation', idevaluation);
      if (error) {
  ...
  async function suppressionComposer(idevaluation: string) {
    try {
      ...
      const { data, error } = await supabase.from('composer').delete().eq('idevaluation', idevaluation);
      if (error) {
  ...
  async function suppressionEvaluation(idevaluation: string) {
    try {
      ...
      const { data, error } = await supabase.from('evaluation').delete().eq('idevaluation', idevaluation);
      if (error) {
  ...
  async function suppressionLink(idevaluation: string) {
    try {
      ...
      if (!linkError && !composerError) {
        ...
        console.error('Erreur lors de la suppression des données de la table link :', deletionError);
        }
      } else {
        console.error('Erreur lors de la récupération des données req1 link:', linkError || composerError);
      }
    } catch (error) {
...
<table>
  ...
  <tbody>
      <tr>
          <td>
            <a href="../rh/noter?idevaluation={evalua.idevaluation}&bareme={evalua.totalBareme}">
                {evalua.nom}
            </a>
          </td>
          <td>
          ...
          <td id="manag">
            ...
            <button id="del" on:click={() => suppressionFinale(evalua.idevaluation)}>supprimer</button>
          </td>
      </tr>
  </tbody>
</table>
