<script>
	...
	const addCompetence2 = (event, index) => {
		...
		if (selectedValue) {
			...
			if (competenceData) {
				...
				criteriaList[index].availableCompetences = criteriaList[index].availableCompetences.filter(
					(c) => c.titre !== selectedValue
				);
	...
	const insertData = async () => {
		try {
			...
			const { data: composerData, error: composerError } = await supabase.from('composer').insert([
				{
			...
			for (const criterion of criteriaList) {
				...
				if (composerError) {
					console.error(
						'Erreur lors de l\'insertion des données pour un critère dans la table "composer":',
						composerError
					);
					return;
				}
				...
				const currentComposerId =
					lastcomposer && lastcomposer.length > 0 ? lastcomposer[0].idcomposer : null;
...
<body>
	...
	{:then}
		...
		{:then}
			...
			<script>
				document.title = 'Cafétence | Création Evaluation';
			</script>
		{:catch error}
			<section class="container d-flex align-items-center justify-content-center vh-100">
